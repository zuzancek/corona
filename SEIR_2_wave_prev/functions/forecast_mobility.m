...
z0 = min2*(gr2_m/100+1)^(tt1-dw2+fcast_days);
...
y0 = 0*y0+zz;
z1 = min2*(0.5*(gr2(end)+gr2(end-1))+1)^(tt1-dw2+fcast_days);
...
y3(tw2:tt1) = w2; 
y3(tt1+1:end) = w2(dw2)+y3(tt1+1:end).*(1+0.75*[0:fcast_days-1]'/fcast_days);
...
y4(tt1+1:end) = y3(tt1+floor(s.smooth_width/2)-1);  
y4(tw2:tt1) = w2; 
