...
p_T_sick_o = pdf(s.T_sick_pdf_type,x_sick_mat,repmat(T_sick_o_shape,1,k_sick),repmat(T_sick_scale,T_total,k_sick));p_T_sick_o = p_T_sick_o./sum(p_T_sick_o,2);
...
T_death_o = s.T_death_o;    p_T_death_o = pdf('Geometric',x_death,1/T_death_o+zeros(k_death,1)); 
T_death_y = s.T_death_y;    p_T_death_y = pdf('Geometric',x_death,1/T_death_y+zeros(k_death,1)); 
omega_o = p.omega_o;        alpha_hdo = omega_o./x_death.*p_T_death_o;  alpha_hdo = alpha_hdo(end:-1:1); 
omega_y = p.omega_y;        alpha_hdy = omega_y./x_death.*p_T_death_y;  alpha_hdy = alpha_hdy(end:-1:1); 
...
for t=delay+1:T_total 
    ...
    d_H_D_o(t) = dot(alpha_hdo,H_o(t-k_death:t-1));    d_H_R_o(t) = dot(alpha_hro,H_o(t-k_rec:t-1));
    H_o(t) = H_o(t-1)+d_I_H_o(t)-d_H_D_o(t)-d_H_R_o(t);
    d_H_D_y(t) = dot(alpha_hdy,H_y(t-k_death:t-1));    d_H_R_y(t) = dot(alpha_hry,H_y(t-k_rec:t-1));
