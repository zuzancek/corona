...
T_hosp0 = 3+T_symp; % s.T_hosp.mean;            % 4;
T_hosp1 = 4+T_symp; % s.T_hosp.mean;            % 4;
T = dateTo-dateFrom+1;
sigma = 0*sigma(dateFrom:dateTo);
...
omega_icu = 2*4.92/100;       T_rec_icu = 12.8;   x_icu = smooth_series(h.ICU(dateFrom:dateTo),s.smooth_width,s.smooth_type,s.smooth_ends);
omega_norm = 0.5*2.71/100;      T_rec_norm = 7.9;   x_norm = H-x_icu-x_vent;
...
lambda = 0.075;%s.lambda;                  % 6.37/100
...
% calculation
for t = 1:T-2
    d_Is_H(t) = H(t+1)-H(t)+d_H_R(t)+d_H_D(t);
    d_Is_H(t+1) = H(t+2)-H(t+1)+d_H_R(t+1)+d_H_D(t+1);
