function [X,I,obs_ratio_adj,sa,p] = adjust_infection_hospitals_full_x(x,h,s,dateFrom,dateTo,t0,t1,sigma)
...
T_rec_norm = 8.69; %6.9;
T_icu = 8-T_hosp-2;
T_rec_icu = 12.3-T_icu+1.5;
T_vent = 5-T_icu;
T_rec_vent = 14.5-(T_vent+T_icu)+2;
T_death = 12-(T_vent+T_icu);
s.lambda = 0.2133*0.47; %6.52/100;
T_hosp = 2.65+s.T_pre.mean;
lambda = s.lambda; % 6.37/100;
alpha_in = lambda./T_hosp.*ones(T-1,1);
T_inf = s.SI.mean;
alpha_ir = (1-lambda)/T_inf;
alpha_xn = 5.72/100;
alpha_xn = 0.1912/2.79; %
alpha_xn = 0.2133/(2.65);
