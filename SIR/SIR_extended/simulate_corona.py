...
def simul(beta_list,Trec_list,public_trans_mat): 
    ...
    rec_mat = np.array(sample(T_list,x.N_per*x.N_locs)).reshape((x.N_locs,x.N_per))
    ...
    while cont:
        try:
            for j in (range(x.N_per)): 
                ...
                public_trans_vec = public_trans_mat[:,j]
                # Matice infekcii
