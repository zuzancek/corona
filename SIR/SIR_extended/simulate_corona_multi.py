...
def simul(public_trans): 
    ...
    beta_mat = np.random.gamma(xx.beta/xx.beta_scale,xx.beta_scale,size=[x.N_locs,x.N_per])
    rec_mat = np.random.gamma(xx.Trec_mean/xx.gamma_scale,xx.gamma_scale,size=[x.N_locs,x.N_per])
    gamma_mat = 1.0/rec_mat
    print(np.average(gamma_mat[:,0]))
